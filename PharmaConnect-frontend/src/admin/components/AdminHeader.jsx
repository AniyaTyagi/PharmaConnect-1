...
import { Bell, ShieldCheck, Users, Package, CheckCheck, Trash2 } from "lucide-react";
import { useAdmin } from "../context/AdminContext";

const TYPE_META = {
  user:    { icon: Users,      bg: "bg-blue-100",   color: "text-blue-500" },
  product: { icon: Package,    bg: "bg-green-100",  color: "text-green-500" },
  info:    { icon: ShieldCheck,bg: "bg-purple-100", color: "text-purple-500" },
};
...
const AdminHeader = () => {
  ...
  const { stats, notifications, markAllRead, markRead, clearNotifications } = useAdmin();
  ...
  return (
    <header className="flex items-center justify-between px-6 py-3 bg-white border-b border-gray-100 relative z-20">
      <div className="flex items-center gap-2">
        <span className="text-sm font-semibold text-gray-800">Welcome back, {user?.name || "Admin"}!</span>
        <span className="text-[11px] bg-emerald-100 text-emerald-700 font-medium px-2 py-0.5 rounded-full">✓ Verified</span>
      </div>

      <div className="flex items-center gap-3" ref={ref}>
        <div className="relative">
          <button onClick={() => { setOpen((v) => !v); setShowProfile(false); }}
            ...
            <div className="absolute right-0 top-10 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                <div className="flex items-center gap-2">
                  ...
                  {stats.unread > 0 && (
                    <span className="text-[10px] bg-red-100 text-red-600 font-semibold px-1.5 py-0.5 rounded-full">{stats.unread} new</span>
                  )}
                </div>
                <div className="flex gap-1">
                  {notifications.length > 0 && (<>
                    ...
                    <button onClick={clearNotifications} title="Clear all" className="p-1 rounded hover:bg-gray-100 text-gray-400 hover:text-red-500 transition-colors"><Trash2 size={14} /></button>
                  </>)}
                </div>
              </div>
        ...
        <div className="relative" ref={profileRef}>
          <button
            onClick={() => { setShowProfile((v) => !v); setOpen(false); }}
          ...
          {showProfile && (
            <div className="absolute right-0 top-10 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
              ...
              <div className="py-1">
                <button onClick={() => { setShowProfile(false); navigate("/admin/settings"); }}
