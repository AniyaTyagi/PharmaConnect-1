...
import { Bell, Package, ShoppingCart, CheckCheck, Trash2 } from "lucide-react";
import { useSeller } from "../context/SellerContext";

const TYPE_ICON = {
  order:   { icon: ShoppingCart, bg: "bg-blue-100",    color: "text-blue-500" },
  product: { icon: Package,      bg: "bg-emerald-100", color: "text-emerald-500" },
};
...
const SellerHeader = () => {
  ...
  const { settings, stats, notifications, markAllRead, markRead, clearNotifications } = useSeller();
  const [open,        setOpen]        = useState(false);
  const [showProfile, setShowProfile] = useState(false);
  const ref        = useRef(null);
  const profileRef = useRef(null);

  // Fallback to JWT user if settings not yet populated
  const user    = JSON.parse(localStorage.getItem("user") || "{}");
  const name    = settings?.name  || user?.name  || "Seller";
  ...
  return (
    <header className="flex items-center justify-between px-6 py-3 bg-white border-b border-gray-100 relative z-20">
      <div className="flex items-center gap-2">
        <span className="text-sm font-semibold text-gray-800">Welcome back, {name}!</span>
        <span className="flex items-center gap-1 text-[11px] bg-emerald-100 text-emerald-700 font-medium px-2 py-0.5 rounded-full">
          ✓ Verified
        </span>
      </div>
            ...
            <div className="absolute right-0 top-10 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
              ...
              <div className="max-h-80 overflow-y-auto">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n) => {
                    const meta = TYPE_ICON[n.type] || TYPE_ICON.product;
        ...
        <div className="relative" ref={profileRef}>
          ...
          {showProfile && (
            <div className="absolute right-0 top-10 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
              ...
              <div className="py-1">
                <button onClick={() => { setShowProfile(false); navigate("/seller/settings"); }}
                  className="w-full text-left px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 transition-colors">Settings</button>
                <button onClick={() => { setShowProfile(false); navigate("/seller/orders"); }}
